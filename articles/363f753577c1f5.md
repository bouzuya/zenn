...
title: "axum crate の Middleware (1) Middleware の適用順序"
topics: ["rust"]
type: "tech"
---
...
`tower::ServiceBuilder` は逆の上から下 (内側から外側) の順。これは紛らわしいですね……。より直感的な順序になるのは、 `ServiceBuilder` を推奨する理由のひとつとのこと。
...
自身は内側、引数は外側に……という処理になっています。

全体としては「引数を外側に」で wrap していく形ですね。
...
引数を自身よりも内側に……を繰り返す形になっています。

全体では「引数を内側に」で、一番内側にはめこんでいく形ですね。

## 実際に試してみる

ごちゃごちゃ考えるより実際に書いてみると良いですね。

<https://github.com/bouzuya/rust-examples/blob/097e4444669f425712f1693536ce474289800001/axum8/src/main.rs>

小さい例を書いて確かにドキュメント通りの順序になることを確認できました。

## おわりに

今回は axum の middleware のドキュメント (前半) を見ました。特に `ServiceBuilder::layer` や `Router::layer` の適用順序の違いに注目しました。

次回はドキュメントの後半で、 Middleware を書いてみたいと思います。
