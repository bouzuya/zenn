...
axum の GitHub リポジトリの examples には testing があります。

<https://github.com/tokio-rs/axum/tree/axum-v0.8.6/examples/testing>
...
`tower` crate は `util` feature を有効にすることで `ServiceExt` trait を使用できます。ここには `oneshot` メソッドがあり、単発のリクエストを処理できます。
...
[`axum::response::Response::into_body`](https://docs.rs/axum/0.8.6/axum/response/type.Response.html#method.into_body) は `Response<T>` の `T` を返します。既定値は [`axum::body::Body`](https://docs.rs/axum/0.8.6/axum/body/struct.Body.html) です。

[`axum::body::Body::collect`](https://docs.rs/axum/0.8.6/axum/body/struct.Body.html#method.collect) は [`http_body_util::combinators::Collect`](https://docs.rs/http-body-util/0.1.3/http_body_util/combinators/struct.Collect.html) を返します。 `Collect` は `Future` を実装しており、 `Result<Collected<<T as Body>::Data>, <T as Body>::Error>` を返します。
...
[`axum::body::to_bytes`](https://docs.rs/axum/0.8.6/axum/body/fn.to_bytes.html) は `axum::body::Body` から `bytes::Bytes` に変換するものです。

`response.into_body().collect().await.unwrap().to_bytes()` よりは `axum::body::to_bytes(response.into_body(), usize::MAX).await.unwrap()` のほうが読みやすくないですか？　好みの問題ですかね……。
